...
func NewContainer(ctx context.Context, opts ...Option) (*Container, error) {
	...
	go func() {
		...
		// Копируем логи в выходной поток (блокирующая операция)
		_, err = io.Copy(cfg.LogOutput, logs)
...
func DefaultHostConfig() func(hc *container.HostConfig) {
	return func(hc *container.HostConfig) {
		// Отключаем AutoRemove для отладки E2E тестов
		// Это позволяет посмотреть логи упавшего контейнера
		hc.AutoRemove = false
	}
}
