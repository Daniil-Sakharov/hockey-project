...
func GenerateLineChart(labels []string, datasets []LineDataset, opts *LineChartOptions) string {
	...
	// Находим максимальное значение среди всех datasets
	maxVal := 0
...
// calculateLinePoints вычисляет координаты точек
...
// buildLinePath строит SVG path для линии
...
// buildAreaPath строит SVG path для заливки области
...
// generateLineGrid генерирует сетку для линейного графика
