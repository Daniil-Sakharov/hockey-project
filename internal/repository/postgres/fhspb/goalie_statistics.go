...
func (r *GoalieStatisticsRepository) GetByTournament(ctx context.Context, tournamentID string) ([]GoalieStatistics, error) {
	var stats []GoalieStatistics
	err := r.db.SelectContext(ctx, &stats,
		`SELECT id, player_id, team_id, tournament_id, games, minutes, goals_against, shots_against, save_percentage, goals_against_avg, wins, shutouts, assists, penalty_minutes, created_at, updated_at 
