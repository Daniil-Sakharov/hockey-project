...
func (r *PlayerStatisticsRepository) Upsert(ctx context.Context, s *PlayerStatistics) error {
	// Используем INSERT с проверкой существования вместо ON CONFLICT
	// так как у нас partial unique index
	query := `
...
func (r *PlayerStatisticsRepository) GetByTournament(ctx context.Context, tournamentID string) ([]PlayerStatistics, error) {
	var stats []PlayerStatistics
	err := r.db.SelectContext(ctx, &stats,
		`SELECT id, player_id, team_id, tournament_id, games, points, points_avg, goals, assists, plus_minus, penalty_minutes, penalty_avg, created_at, updated_at 
		FROM player_statistics WHERE tournament_id = $1 ORDER BY points DESC`,
		tournamentID,
	)
	return stats, err
}
