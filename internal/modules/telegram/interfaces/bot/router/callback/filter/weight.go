...
func HandleWeight(r Router, ctx context.Context, bot *tgbotapi.BotAPI, query *tgbotapi.CallbackQuery, parts []string) {
	...
	if subCmd == cb.SubCmdSelect {
		...
		if err := r.FilterHandler().HandleWeightValue(ctx, bot, query, subCmd); err != nil {
			logger.Error(ctx, "Error handling weight value", zap.Error(err), zap.String("value", subCmd))
		}
	}
}
