...
// PlayerProfileDTO данные из профиля игрока
type PlayerProfileDTO struct {
	Position    string // Амплуа
	Height      string // Рост
	Weight      string // Вес
	Handedness  string // Хват
	Citizenship string // Гражданство
...
type TeamDTO struct {
	...
	City    string // Город
	LogoURL string // Полный URL логотипа
}
...
type TournamentDTO struct {
	...
	BirthYears []int  // Годы рождения со страницы списка турниров (fallback)
}
...
// GroupLink информация о группе/этапе из AJAX-ссылки
type GroupLink struct {
	Name    string // Название группы (А1, А2, Г и т.д.)
	AjaxURL string // AJAX URL для загрузки команд этой группы
}

// TeamWithContext команда с информацией о годе рождения и группе
type TeamWithContext struct {
	Team      TeamDTO
	BirthYear *int    // Год рождения для данной команды в турнире
	GroupName *string // Название группы/этапа для данной команды в турнире
}
