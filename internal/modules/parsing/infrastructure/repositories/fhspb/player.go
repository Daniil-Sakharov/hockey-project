...
func (r *PlayerRepository) Upsert(ctx context.Context, p *Player) (string, error) {
	...
	query := `
		INSERT INTO players (id, external_id, name, profile_url, birth_date, birth_place, position, height, weight, handedness, source, region, created_at, updated_at)
	...
	err := r.db.QueryRowContext(ctx, query, id, p.ExternalID, p.FullName, p.ProfileURL, p.BirthDate, p.BirthPlace, p.Position, p.Height, p.Weight, p.Handedness, SourceFHSPB, RegionSPB).Scan(&returnedID)
	return returnedID, err
}

func (r *PlayerRepository) GetByExternalID(ctx context.Context, externalID string) (*Player, error) {
	var p Player
	err := r.db.GetContext(ctx, &p, `SELECT id, external_id, name, profile_url, birth_date, birth_place, position, height, weight, handedness, source, region, created_at, updated_at FROM players WHERE external_id = $1 AND source = $2`, externalID, SourceFHSPB)
	if err != nil {
		return nil, err
	}
	return &p, nil
