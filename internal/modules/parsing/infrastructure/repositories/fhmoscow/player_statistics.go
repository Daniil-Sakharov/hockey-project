...
func (r *PlayerStatisticsRepository) Upsert(ctx context.Context, s *PlayerStatistics) error {
	// Используем 0 для NULL значений (колонки NOT NULL в БД)
	goalsPP := 0
	goalsSH := 0
	goalsES := 0
	if s.GoalsPowerPlay != nil {
		goalsPP = *s.GoalsPowerPlay
	}
	if s.GoalsShortHanded != nil {
		goalsSH = *s.GoalsShortHanded
	}
	if s.GoalsEvenStrength != nil {
		goalsES = *s.GoalsEvenStrength
	}
