...
func (wp *TournamentWorkerPool) worker(workerID int) {
	...
	for task := range wp.tasks {
		...
		// Формируем правильный URL для логирования
		var displayURL string
		if strings.HasPrefix(task.Tournament.URL, "http") {
			// Абсолютный URL (FHSPB)
			displayURL = task.Tournament.URL
		} else {
			// Относительный URL (Junior)
			displayURL = task.Tournament.Domain + task.Tournament.URL
		}

		wp.service.logger.Printf("[Worker %d]         URL: %s",
			workerID,
			displayURL)
